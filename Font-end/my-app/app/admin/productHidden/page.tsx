...
export default function ShowAdmin() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await fetch("http://localhost:5000/products/inactive", {
        method: "GET",
        headers: {
  ...
  const handleShowProduct = async (productId: string) => {
    try {
      const response = await fetch(
        `http://localhost:5000/products/show/${productId}`,
        {
          method: "PUT",
          headers: {
            Accept: "application/json",
      ...
      if (!response.ok) {
        throw new Error("Lỗi khi hiện sản phẩm");
      }

      // Cập nhật lại danh sách sản phẩm sau khi hiện
      await fetchPosts();
      toast.success("Hiện sản phẩm thành công!");
    } catch (error: any) {
      console.error("Lỗi khi hiện sản phẩm:", error);
      toast.error(error.message || "Không thể hiện sản phẩm");
    }
  };
