...
exports.getCommentableProducts = async (req, res) => {
  try {
    ...
    const commentedProductIds = commentedProducts.map(comment => comment.productId.toString());
    ...
    return res.status(200).json({
      ...
      canCommentOrder: !hasOrderComment, // Cho phép bình luận đơn hàng nếu chưa có bình luận chung
    });
  } catch (err) {
    console.error("Lỗi khi lấy sản phẩm có thể bình luận:", err);
